...
\begin{figure}[tp]\centering
    \subfloat[]
    {
        ...
        \includegraphics[scale=0.17]{fig/yawing.pdf}
        \end{minipage}
    }\subfloat[]
    {
        \begin{minipage}{4cm}\centering
...
To detect vehicle yawing effectively, we desire to be able to forecast the driving intention of the vehicle before it enters the fork road. 
However, we cannot obtain information through GPS about whether the vehicle is yawing in advance. 
By observing other sensor data, we discovered that both IMU and image contain predictive information for yawing.
In addition, compared to the lane change information from the IMU that does not appear stable, the image can always reveal the road the vehicle is about to enter.
Therefore, as long as images from ubiquitous dashcams are successfully used, we can achieve the predictive ability of vehicle yawing by predicting driving intentions.
...
$Left$ means that the vehicle drives towards left when facing to the fork road, and $right$ is opposite.
